...
$total = $Rcost * $duration1;
	...
	//Selecting database
		...
		if($numrows == 0)
		{
			...
			mysqli_query($mysqli, "INSERT INTO accounting_invoices(inv_number,source,staff,item_desc,quantity,price,total,month,added_date) VALUES ('".$inv_num."','".$source1."','".$rstaff1."','".$room1."','".$duration1."','".$Rcost."','".$total."',MONTHNAME(CURRENT_DATE),NOW())");
			...
			if($result){
				...
				header("Location: ../payment-confirm.php?name=$cname1&room_number=$room1&room_type=$Rtype&total=$total&date_checkin=$c_in_date1&date_checkout=$c_out_date1&inv_num=$inv_num");
